...
public class DlgBilingRalan extends javax.swing.JDialog {
    ...
    private PreparedStatement pscaripoli2,pscekbilling,pscarirm,pscaripasien,psreg,pscaripoli,pscarialamat,psrekening,
            ...
            pstamkur,psnota,psoperasi,psobatoperasi,psakunbayar,psakunpiutang,psceklab,psceklabpa,pscekrad,pscekresep,pscarilabpa;
    private ResultSet rscekbilling,rscarirm,rscaripasien,rsreg,rscaripoli,rscarialamat,rsrekening,rsobatoperasi,
            ...
            rsobatlangsung,rsreturobat,rstambahan,rspotongan,rsbilling,rscariradiologi,rstamkur,rsoperasi,nota,
            rsakunbayar,rsakunpiutang,rscaripoli2, rsceklab, rscekrad, rscekresep,rsceklabpa,rscarilabpa;
    ...
    private void BtnNotaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnNotaActionPerformed
            if(TNoRw.getText().trim().equals("")||TNoRM.getText().trim().equals("")||TPasien.getText().trim().equals("")){
                ...
                try{
                    ...
                    i = 0;
                    try{
                          biaya = (String)JOptionPane.showInputDialog(null,"Silahkan pilih nota yang mau dicetak!","Nota",JOptionPane.QUESTION_MESSAGE,null,new Object[]{"Nota", "Kwitansi", "Nota & Kwitansi","Kwitansi Piutang","Nota MCU"},"Nota");
                          switch (biaya) {
                                case "Nota":
                                      i=1;
                                      break;
                    ...
                    if(i>0){                       
                        ...
                        if(i==1){
                            ...
                            if(piutang>0){
                                Valid.panggilUrl("billing/LaporanBillingMCU.php?petugas="+akses.getkode().replaceAll(" ","_")+"&nonota="+Sequel.cariIsi("select count(reg_periksa.no_rawat) from reg_periksa "+
                                        "where reg_periksa.kd_pj='"+kd_pj+"' and reg_periksa.tgl_registrasi like '%"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,7)+"%'")+"/RJ/"+kd_pj+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(5,7)+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,4)); 
                            }else if(piutang<=0){
                                Valid.panggilUrl("billing/LaporanBillingMCU.php?petugas="+akses.getkode().replaceAll(" ","_")+"&nonota="+Sequel.cariIsi("select count(reg_periksa.no_rawat) from reg_periksa "+
                                        "where reg_periksa.kd_pj='"+kd_pj+"' and reg_periksa.tgl_registrasi like '%"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,7)+"%'")+"/RJ/"+kd_pj+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(5,7)+"/"+Valid.SetTgl(DTPTgl.getSelectedItem()+"").substring(0,4));
...
//            cek permintaan
            try {
                psceklab=koneksi.prepareStatement("select count(permintaan_lab.tgl_hasil) as status from permintaan_lab where permintaan_lab.tgl_hasil='0000-00-00' and permintaan_lab.status='ralan' and permintaan_lab.no_rawat=?");
                pscekrad=koneksi.prepareStatement("select count(permintaan_radiologi.tgl_hasil) as status from permintaan_radiologi where permintaan_radiologi.tgl_hasil='0000-00-00' and permintaan_radiologi.status='ralan' and permintaan_radiologi.no_rawat=?");
                pscekresep=koneksi.prepareStatement("select count(resep_obat.tgl_perawatan) as status from resep_obat where resep_obat.tgl_perawatan='0000-00-00' and resep_obat.status='ralan' and resep_obat.no_rawat=?");
                psceklabpa=koneksi.prepareStatement("select count(permintaan_labpa.tgl_hasil) as status from permintaan_labpa where permintaan_labpa.tgl_hasil='0000-00-00' and permintaan_labpa.status='ralan' and permintaan_labpa.no_rawat=?");
