...
public final class DlgKasirRalan extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        MnCetakBarcode = new javax.swing.JMenuItem();
        ...
        MnCetakBarcode.setBackground(new java.awt.Color(255, 255, 254));
        MnCetakBarcode.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnCetakBarcode.setForeground(new java.awt.Color(50, 50, 50));
        MnCetakBarcode.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnCetakBarcode.setText("Cetak Barcode RM");
        MnCetakBarcode.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        MnCetakBarcode.setHorizontalTextPosition(javax.swing.SwingConstants.RIGHT);
        MnCetakBarcode.setName("MnCetakBarcode"); // NOI18N
        MnCetakBarcode.setPreferredSize(new java.awt.Dimension(210, 26));
        MnCetakBarcode.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnCetakBarcodeActionPerformed(evt);
            }
        });
        jPopupMenu2.add(MnCetakBarcode);
        ...
        TglSakit1.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "24-06-2022" }));
        ...
        TglSakit2.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "24-06-2022" }));
...
private void MnDataPemberianObatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnDataPemberianObatActionPerformed
    ...
    private void MnMeninggalActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnMeninggalActionPerformed
        if(TNoRw.getText().trim().equals("")){
            ...
            if(Sequel.cariInteger("select count(no_rawat) from kamar_inap where no_rawat=?",TNoRw.getText())>0){
                ...
                dlgPasienMati.setTglMasuk(tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),12).toString());
                dlgPasienMati.setJamMasuk(tbKasirRalan.getValueAt(tbKasirRalan.getSelectedRow(),13).toString());
    ...
    private void MnSJPActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnSJPActionPerformed
        if(tabModekasir.getRowCount()==0){
            ...
            dlgki.setNoRm(TNoRw.getText(),Valid.SetTgl2(Tanggal.getText()),"1 RJTP RAWAT JALAN TINGKAT PERTAMA",kdpoli.getText(),nmpoli.getText());   
            dlgki.setVisible(true);
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    private void MnBarcode2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnBarcode2ActionPerformed
        if(TPasienCari.getText().trim().equals("")){
            ...
            param.put("kontakrs",akses.getkontakrs());
            param.put("emailrs",akses.getemailrs());
            param.put("umurnow",Sequel.cariIsi("SELECT umur FROM pasien join reg_periksa on reg_periksa.no_rkm_medis = pasien.no_rkm_medis WHERE reg_periksa.no_rawat=?",TNoRw.getText()));
            param.put("no_rawat",TNoRw.getText());
            param.put("logo",Sequel.cariGambar("select logo from setting"));
    ...
    private void MnCetakBarcodeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakBarcodeActionPerformed
        // TODO add your handling code here:
       if(tabModekasir.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data pasien sudah habis...!!!!");
            TNoRMCari.requestFocus();
            ...
            Valid.MyReportqry("rptBarcodeRawat3Internal.jasper","report","::[ Barcode Internal ]::","select pasien.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, pasien.jk, "+
                "pasien.tmp_lahir, pasien.tgl_lahir, pasien.umur, pasien.nm_ibu, concat(pasien.alamat,', ',kelurahan.nm_kel,', ',kecamatan.nm_kec,', ',kabupaten.nm_kab) as alamat, pasien.gol_darah, pasien.pekerjaan,"+
                "pasien.stts_nikah,pasien.agama,pasien.tgl_daftar,pasien.no_tlp,"+
                "pasien.pnd, pasien.keluarga, pasien.namakeluarga,penjab.png_jawab,pasien.pekerjaanpj,"+
                "concat(pasien.alamatpj,', ',pasien.kelurahanpj,', ',pasien.kecamatanpj,', ',pasien.kabupatenpj) as alamatpj, reg_periksa.no_rawat, rujukan_internal_poli.kd_poli, dokter.nm_dokter from pasien "+
                "inner join kelurahan inner join kecamatan inner join kabupaten "+
                "inner join penjab on pasien.kd_pj=penjab.kd_pj and pasien.kd_kel=kelurahan.kd_kel "+
                ...
                "INNER JOIN dokter on dokter.kd_dokter = rujukan_internal_poli.kd_dokter where pasien.no_rkm_medis='"+TNoRMCari.getText()+"' ",param);
            this.setCursor(Cursor.getDefaultCursor());
        }
    }//GEN-LAST:event_MnCetakBarcodeActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem MnCetakBarcode;
