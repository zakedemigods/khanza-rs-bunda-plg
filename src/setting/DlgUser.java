...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[L]Penilaian Awal Keperawatan Ranap Umum","[D]Nilai Penerimaan Vendor Farmasi Per Bulan","[J]Akun Bayar Hutang","[I]Hitung BOR DINKES","[L]Master Rencana Keperawatan"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "hitung_bor_dinkes='"+tbUser.getValueAt(i,811).toString()+"',"+
                    "master_rencana_keperawatan='"+tbUser.getValueAt(i,812).toString()+"'");
            }            
            tampil();
            emptTeks();
        }
                            ...
                            try {
                                ...
                                if(i!= -1){
                                    Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                                        ...
                                        "hitung_bor_dinkes='"+tbUser.getValueAt(barisdicopy,811).toString()+"',"+
                                        "master_rencana_keperawatan='"+tbUser.getValueAt(barisdicopy,812).toString()+"'"
                                        );
                                }    
                                userdicopy="";
                                copyhakakses="";
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.pembayaran_briva,user.penilaian_awal_keperawatan_ranap,user.nilai_penerimaan_vendor_farmasi_perbulan,user.akun_bayar_hutang,user.hitung_bor_dinkes,user.master_rencana_keperawatan from user order by AES_DECRYPT(user.id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
                    ...
                    try {
                        if(rs.getString(1).toLowerCase().contains(TCari.getText().toLowerCase())||
                            ...
                            tabMode.addRow(new Object[]{rs.getString(1),
                               ...
                               rs.getBoolean("hitung_bor_dinkes"),
